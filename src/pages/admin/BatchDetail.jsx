...
// PURPOSE: Single batch detail. Enroll modal now includes
//          Aadhaar number field (required, 12 digits).
//          Shows info toast if student is already in another batch.
// =============================================================
...
const BatchDetail = () => {
  ...
  const enroll = async (data) => {
    setSaving(true);
    try {
      // ✅ Auto fallback to batch fee
      const finalData = {
        ...data,
        monthlyFee: data.monthlyFee ? Number(data.monthlyFee) : batch.fee,
      };

      const res = await axios.post(
        `${BASE}/admin/student/create`,
        { ...finalData, batchId: id },
        { headers: h() },
      );
      ...
      if (res.data.info) {
        toast.info(res.data.info, { autoClose: 5000 });
      }
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            {batch.timing || "Timing not set"} · {batch.coachName || "No coach"}
          </p>
        </div>
